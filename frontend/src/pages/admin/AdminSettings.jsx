...
const AdminSettings = () => {
  ...
  return (
    <>
      ...
      <style>{`
        .admin-settings { padding: 1.5rem; background: var(--bg-main); min-height: calc(100vh - 70px); }
        .header { margin-bottom: 2rem; }
        .header h1 { font-size: 1.5rem; color: var(--text-main); margin-bottom: 0.25rem; }
        .header p { color: var(--text-muted); font-size: 0.875rem; }
        .settings-form { background: var(--card-bg); border-radius: 0.75rem; padding: 2rem; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05); border: 1px solid var(--border-color); }
        .settings-section { margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border-color); }
        .settings-section:last-child { border-bottom: none; margin-bottom: 0; padding-bottom: 0; }
        .settings-section h2 { display: flex; align-items: center; gap: 0.5rem; font-size: 1rem; color: var(--text-main); margin-bottom: 1rem; font-weight: 600; }
        .section-icon { width: 1rem; height: 1rem; color: var(--primary); }
        .form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem; }
        .form-group { margin-bottom: 1rem; }
        .form-group label { display: block; font-weight: 500; margin-bottom: 0.5rem; font-size: 0.875rem; color: var(--text-main); }
        .form-group input, .form-group textarea { width: 100%; padding: 0.625rem; border: 1px solid var(--border-color); border-radius: 0.5rem; font-size: 0.875rem; background: var(--bg-main); color: var(--text-main); transition: border-color 0.3s, box-shadow 0.3s; }
        .form-group input:focus, .form-group textarea:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 2px rgba(212, 175, 55, 0.2); }
        .toggle-group { margin-bottom: 1rem; }
        .toggle-group label { font-weight: 500; margin-bottom: 0.5rem; display: block; font-size: 0.875rem; color: var(--text-main); }
        .toggle-group div { display: flex; align-items: center; gap: 0.5rem; }
        .toggle-group input { width: 1rem; height: 1rem; cursor: pointer; accent-color: var(--primary); }
        .toggle-group span { font-size: 0.875rem; color: var(--text-muted); }
        .color-preview { display: flex; align-items: center; gap: 0.5rem; }
        .color-box { width: 2rem; height: 2rem; border-radius: 0.25rem; border: 1px solid var(--border-color); }
        .color-preview span { font-size: 0.875rem; color: var(--text-muted); font-family: monospace; }
        .form-actions { margin-top: 2rem; }
        .btn-save { display: flex; align-items: center; justify-content: center; gap: 0.5rem; width: 100%; padding: 0.75rem; background: var(--primary); color: var(--bg-main); border: none; border-radius: 0.5rem; font-weight: 600; cursor: pointer; transition: all 0.3s; font-size: 0.875rem; }
        .btn-icon { width: 1rem; height: 1rem; }
        .btn-save:hover:not(:disabled) { transform: translateY(-1px); opacity: 0.9; }
