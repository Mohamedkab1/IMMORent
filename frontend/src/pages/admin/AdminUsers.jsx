...
const AdminUsers = () => {
  ...
  const getRoleBadge = (role) => {
    const colors = { admin: { bg: 'bg-red-100 dark:bg-red-900/30', color: 'text-red-700 dark:text-red-500' }, agent: { bg: 'bg-blue-100 dark:bg-blue-900/30', color: 'text-blue-700 dark:text-blue-500' }, client: { bg: 'bg-green-100 dark:bg-green-900/30', color: 'text-green-700 dark:text-green-500' } };
    const c = colors[role?.slug] || colors.client;
    return <span className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${c.bg} ${c.color}`}>{role?.name}</span>;
  };
  ...
  return (
    <>
      ...
      <style>{`
        .admin-users { padding: 1.5rem; background: var(--bg-main); min-height: calc(100vh - 70px); }
        .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; }
        .header h1 { font-size: 1.5rem; color: var(--text-main); }
        .btn-add { display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; background: var(--primary); color: white; border: none; border-radius: 0.5rem; cursor: pointer; }
        ...
        .clear-search { position: absolute; right: 0.5rem; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: var(--text-muted); }
        .search-info { font-size: 0.75rem; color: var(--text-muted); }
        ...
        @media (prefers-color-scheme: dark) {
          .status-toggle.active { background: rgba(5, 150, 105, 0.3); color: #34d399; }
          .status-toggle.inactive { background: rgba(220, 38, 38, 0.3); color: #f87171; }
        }
        .actions {
            display: flex;
            gap: 0.5rem;
        ...
        .modal-content { background: var(--card-bg); border-radius: 0.75rem; width: 90%; max-width: 500px; max-height: 90vh; overflow-y: auto; border: 1px solid var(--border-color); }
        ...
        .form-group label { display: flex; align-items: center; gap: 0.25rem; font-weight: 500; margin-bottom: 0.5rem; font-size: 0.875rem; color: var(--text-main); }
        .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.5rem; background: var(--bg-main); color: var(--text-main); }
        .modal-actions { display: flex; gap: 1rem; padding: 1rem 1.5rem; border-top: 1px solid var(--border-color); }
        .btn-cancel { flex: 1; padding: 0.5rem; background: var(--bg-muted); color: var(--text-main); border: none; border-radius: 0.5rem; cursor: pointer; }
        .btn-submit, .btn-delete { flex: 1; padding: 0.5rem; background: var(--primary); border: none; border-radius: 0.5rem; cursor: pointer; font-weight: 600; color: white; }
