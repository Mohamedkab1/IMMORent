...
const AdminUsers = () => {
  ...
  return (
    <>
      ...
      {showModal && (<div className="modal-overlay" onClick={closeModal}><div className="modal-content" onClick={e => e.stopPropagation()}><div className="modal-header"><h2>{editingUser ? t('common.edit', 'Modifier') : t('admin.add.add_btn', 'Ajouter')}</h2><button className="close-modal" onClick={closeModal}><XMarkIcon /></button></div><form onSubmit={handleSubmit} className="modal-form"><div className="form-group"><label><UserIcon /> {t('auth.name', 'Nom')}</label><input type="text" name="name" value={formData.name} onChange={handleChange} required /></div><div className="form-group"><label><EnvelopeIcon /> {t('auth.email', 'Email')}</label><input type="email" name="email" value={formData.email} onChange={handleChange} required /></div><div className="form-group"><label><PhoneIcon /> {t('admin.users.phone', 'Téléphone')}</label><input type="tel" name="phone" value={formData.phone} onChange={handleChange} /></div><div className="form-group"><label>{t('admin.users.address', 'Adresse')}</label><textarea name="address" rows="2" value={formData.address} onChange={handleChange} /></div><div className="form-group"><label><ShieldCheckIcon /> {t('admin.users.role', 'Rôle')}</label><select name="role_id" value={formData.role_id} onChange={handleChange}>{roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select></div>{!editingUser && (<><div className="form-group"><label>{t('auth.pwd', 'Mot de passe')}</label><div style={{position: 'relative'}}><input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange} required style={{paddingRight: '2.5rem'}} /><button type="button" onClick={() => setShowPassword(!showPassword)} style={{position: 'absolute', right: '0.5rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', padding: '0', display: 'flex', alignItems: 'center'}}>{showPassword ? <EyeSlashIcon style={{width: '1rem', height: '1rem', color: '#6b7280'}} /> : <EyeIcon style={{width: '1rem', height: '1rem', color: '#6b7280'}} />}</button></div></div><div className="form-group"><label>{t('auth.pwd_confirm', 'Confirmation')}</label><div style={{position: 'relative'}}><input type={showConfirmPassword ? "text" : "password"} name="password_confirmation" value={formData.password_confirmation} onChange={handleChange} required style={{paddingRight: '2.5rem'}} /><button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} style={{position: 'absolute', right: '0.5rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', padding: '0', display: 'flex', alignItems: 'center'}}>{showConfirmPassword ? <EyeSlashIcon style={{width: '1rem', height: '1rem', color: '#6b7280'}} /> : <EyeIcon style={{width: '1rem', height: '1rem', color: '#6b7280'}} />}</button></div></div></>)}<div className="modal-actions"><button type="button" className="btn-cancel" onClick={closeModal}>{t('common.cancel', 'Annuler')}</button><button type="submit" className="btn-submit">{editingUser ? t('common.edit', 'Modifier') : t('admin.add.add_btn', 'Ajouter')}</button></div></form></div></div>)}
