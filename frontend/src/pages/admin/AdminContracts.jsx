...
const AdminContracts = () => {
  ...
  const getStatusBadge = (status) => {
    const config = {
      active: { bg: 'bg-green-100 dark:bg-green-900/30', color: 'text-green-700 dark:text-green-500', text: 'Actif' },
      terminated: { bg: 'bg-red-100 dark:bg-red-900/30', color: 'text-red-700 dark:text-red-500', text: 'Résilié' },
      expired: { bg: 'bg-[var(--bg-muted)]', color: 'text-[var(--text-muted)]', text: 'Expiré' }
    };
  ...
  return (
    <>
      ...
      <style>{`
        .admin-contracts { padding: 1.5rem; background: var(--bg-main); min-height: calc(100vh - 70px); }
