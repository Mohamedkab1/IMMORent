      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {[
            ...
            <div key={i} className="bg-[var(--card-bg)] p-6 rounded-3xl shadow-sm border border-[var(--border-color)] text-center hover:shadow-xl hover:-translate-y-2 transition-all duration-300">
              ...
              <h3 className="text-lg font-bold text-[var(--text-main)] mb-3">{info.title}</h3>
              <p className="text-[var(--text-muted)] text-sm leading-relaxed font-medium">
                {info.line1} <br /> {info.line2}
              </p>
              {info.note && <p className="text-xs font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-wider mt-4">{info.note}</p>}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-20">
          ...
          {/* Form */}
          <div className="bg-[var(--card-bg)] p-8 md:p-12 rounded-3xl shadow-sm border border-[var(--border-color)] relative overflow-hidden">
             ...
             <h2 className="text-3xl font-extrabold text-[var(--text-main)] mb-2 relative z-10">Envoyez un message</h2>
             <p className="text-[var(--text-muted)] mb-8 relative z-10">Remplissez ce formulaire et nous vous recontacterons au plus vite.</p>
             ...
             {submitted ? (
               <div className="flex flex-col items-center justify-center py-12 text-center relative z-10 animate-fade-in-up">
                 ...
                 <h3 className="text-2xl font-bold text-[var(--text-main)] mb-4">Message Envoyé !</h3>
                 <p className="text-[var(--text-muted)] mb-8 max-w-sm">Merci de nous avoir contactés. Notre équipe reviendra vers vous très prochainement.</p>
                 <button onClick={() => setSubmitted(false)} className="px-8 py-3 bg-[var(--text-main)] text-[var(--bg-main)] hover:opacity-90 rounded-xl font-bold transition-colors">
               ...
               <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2">
                     <label className="text-sm font-bold text-[var(--text-main)]">Nom complet <span className="text-rose-500">*</span></label>
                     <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="Jean Dupont" className="w-full px-4 py-3 bg-[var(--bg-main)] border appearance-none outline-none border-[var(--border-color)] rounded-xl text-[var(--text-main)] font-medium focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
                   </div>
                   <div className="space-y-2">
                     <label className="text-sm font-bold text-[var(--text-main)]">Email <span className="text-rose-500">*</span></label>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2">
                     <label className="text-sm font-bold text-[var(--text-main)]">Téléphone</label>
                   ...
                   <div className="space-y-2">
                     <label className="text-sm font-bold text-[var(--text-main)]">Sujet <span className="text-rose-500">*</span></label>
                     <select name="subject" value={formData.subject} onChange={handleChange} required className="w-full px-4 py-3 bg-[var(--bg-main)] border outline-none border-[var(--border-color)] rounded-xl text-[var(--text-main)] font-medium focus:border-primary focus:ring-1 focus:ring-primary transition-all appearance-none cursor-pointer">
                       <option value="">Sélectionnez un sujet</option>
                       <option value="info" className="bg-[var(--card-bg)]">Demande d'information</option>
                       <option value="support" className="bg-[var(--card-bg)]">Support technique</option>
                       <option value="partnership" className="bg-[var(--card-bg)]">Partenariat</option>
                       <option value="claim" className="bg-[var(--card-bg)]">Réclamation / Signalement</option>
                       <option value="visit" className="bg-[var(--card-bg)]">Demande de visite</option>
                     </select>
                   </div>
                 </div>

                 <div className="space-y-2">
                   <label className="text-sm font-bold text-[var(--text-main)]">Votre message <span className="text-rose-500">*</span></label>
                   <textarea name="message" value={formData.message} onChange={handleChange} rows="5" required placeholder="Comment pouvons-nous vous aider ?" className="w-full px-4 py-3 bg-[var(--bg-main)] border appearance-none outline-none border-[var(--border-color)] rounded-xl text-[var(--text-main)] font-medium focus:border-primary focus:ring-1 focus:ring-primary transition-all resize-y" />
                 </div>

                 <div className="flex items-start gap-3 mt-4">
                   <input type="checkbox" id="consent" required className="mt-1 w-5 h-5 rounded border-[var(--border-color)] text-primary focus:ring-primary dark:border-slate-600 dark:bg-[var(--bg-muted)]" />
                   <label htmlFor="consent" className="text-sm text-[var(--text-muted)] font-medium">
          ...
          {/* Map */}
          <div className="h-[400px] lg:h-full min-h-[500px] w-full rounded-3xl overflow-hidden shadow-sm border border-[var(--border-color)] relative group">
            {/* Map Placeholder or Google maps Embed */}
            <div className="absolute inset-0 bg-[var(--bg-muted)] animate-pulse pointer-events-none -z-10"></div>
            <iframe 
        ...
        <div className="text-center max-w-4xl mx-auto">
          <h2 className="text-3xl font-extrabold text-[var(--text-main)] inline-block relative mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-start">
            {[
              ...
              <div key={i} className="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-color)] shadow-sm hover:shadow-md transition-shadow">
                <h3 className="text-lg font-bold text-[var(--text-main)] mb-3 flex items-start gap-3">
                   <div className="mt-1 w-2 h-2 rounded-full bg-secondary flex-shrink-0"></div>
                   {faq.q}
                </h3>
                <p className="text-[var(--text-muted)] font-medium pl-5">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
