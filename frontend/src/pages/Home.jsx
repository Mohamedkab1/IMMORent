    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="py-24 bg-bg-main">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-text-main mb-4">{t('home.categories.title')}</h2>
            ...
            <Link to="/properties" className="hidden md:inline-flex items-center text-primary dark:text-secondary font-semibold hover:gap-2 transition-all">
              {t('home.categories.see_all')} <ArrowRightIcon className="w-4 h-4 ms-1 rtl:rotate-180" />
            </Link>
          </div>
              ...
              <Link to={`/properties?type=${cat.type}`} key={i} className="group relative h-80 rounded-2xl overflow-hidden shadow-md">
                ...
                <div className="absolute bottom-0 left-0 p-6 w-full transform transition-transform duration-300">
                  ...
                  <span className="inline-flex items-center text-sm text-slate-300 group-hover:text-white transition-colors">
                    {t('home.categories.discover')} <ArrowRightIcon className="w-4 h-4 ms-1 opacity-0 -translate-x-2 rtl:translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all rtl:rotate-180" />
                  </span>
                </div>
              </Link>
            ))}
      ...
      <section className="py-20 relative overflow-hidden bg-primary dark:bg-slate-800">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center text-white">
          ...
          <p className="text-lg md:text-xl text-slate-300 mb-10 max-w-2xl mx-auto">{t('home.cta.desc')}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
             <Link to="/register" className="px-8 py-4 bg-secondary hover:bg-secondary-hover text-primary font-bold rounded-xl shadow-lg transition-all duration-300 hover:-translate-y-1">{t('home.register_free')}</Link>
             <Link to="/contact" className="px-8 py-4 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 rounded-xl font-bold shadow-lg transition-all duration-300 hover:-translate-y-1">{t('home.contact_team')}</Link>
          </div>
        </div>
      </section>
