...
const AgentDashboard = () => {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      rented: { bg: 'bg-[var(--bg-muted)]', text: 'text-[var(--text-muted)]', label: t('status.rented', 'Loué'), icon: XCircleIcon },
    ...
    <div className="flex flex-col md:flex-row min-h-[calc(100vh-70px)] bg-[var(--bg-main)]">
      ...
      <aside className="w-full md:w-72 bg-[var(--card-bg)] border-e border-[var(--border-color)] flex flex-col transition-colors duration-300">
        <div className="p-6 border-b border-[var(--border-color)] text-center relative pt-12">
           <div className="w-20 h-20 absolute -top-10 left-1/2 -translate-x-1/2 bg-gradient-to-tr from-primary to-blue-400 rounded-2xl flex items-center justify-center text-white text-3xl font-black shadow-xl shadow-primary/30 border-4 border-[var(--card-bg)]">
            {user?.name?.charAt(0)}
          </div>
          <h3 className="text-xl font-bold text-[var(--text-main)] truncate mt-2">{user?.name}</h3>
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button 
            ...
            className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-300 ${activeTab === 'dashboard' ? 'bg-primary dark:bg-secondary text-white dark:text-primary shadow-md' : 'text-[var(--text-muted)] hover:bg-[var(--bg-muted)] hover:text-[var(--text-main)]'}`}
          >
          ...
          <button 
            ...
            className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-300 relative ${activeTab === 'properties' ? 'bg-primary dark:bg-secondary text-white dark:text-primary shadow-md' : 'text-[var(--text-muted)] hover:bg-[var(--bg-muted)] hover:text-[var(--text-main)]'}`}
          >
            <HomeIcon className="w-5 h-5" /> {t('nav.properties')}
            {stats.totalProperties > 0 && <span className="absolute end-3 top-1/2 -translate-y-1/2 px-2 py-0.5 bg-[var(--bg-muted)] text-[var(--text-main)] text-xs rounded-full font-bold">{stats.totalProperties}</span>}
          </button>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-[var(--text-main)] tracking-tight">{t('dash.agent.welcome')}</h1>
            <p className="text-sm text-[var(--text-muted)] mt-1">Supervisez votre portfolio immobilier en temps réel.</p>
          </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Dernières Demandes */}
              <section className="bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-sm p-6 overflow-hidden">
                ...
                <div className="space-y-4">
                  {requests.filter(r => r.status?.toLowerCase() === 'pending').slice(0, 3).map(request => (
                    <div key={request.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-[var(--bg-muted)] rounded-xl border border-[var(--border-color)] gap-4">
                       <div>
                         <h4 className="font-bold text-[var(--text-main)] text-sm">{request.user?.name || 'Client Inconnu'}</h4>
                         <p className="text-xs text-[var(--text-muted)] mb-1">{request.property?.title}</p>
                       </div>
                  ...
                  {requests.filter(r => r.status?.toLowerCase() === 'pending').length === 0 && (
                    <div className="p-6 text-center text-[var(--text-muted)] border-2 border-dashed border-[var(--border-color)] rounded-xl">
              ...
              <section className="bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-sm p-6 overflow-hidden">
                ...
                <div className="space-y-4">
                  {contracts.slice(0, 3).map(contract => (
                    <div key={contract.id} className="flex items-center justify-between p-4 bg-[var(--bg-muted)] rounded-xl border border-[var(--border-color)]">
                       <div>
                         <h4 className="font-bold text-[var(--text-main)] text-sm">{contract.property?.title}</h4>
                         <p className="text-xs text-[var(--text-muted)] mt-1">Locataire: {contract.tenant?.name}</p>
                       </div>
                       <div className="text-right">
                         <div className="font-bold text-[var(--text-main)] text-sm">{contract.monthly_rent?.toLocaleString()} DH</div>
...
{/* Properties Tab */}
        {activeTab === 'properties' && (
          <section className="bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-sm overflow-hidden">
             <div className="p-6 border-b border-[var(--border-color)] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[var(--bg-muted)]">
                <h2 className="text-lg font-bold text-[var(--text-main)]">Gestion de mes biens</h2>
               ...
               <table className="w-full text-left border-collapse">
                 ...
                 <tbody className="divide-y divide-[var(--border-color)]">
                   {properties.map(property => (
                     <tr key={property.id} className="hover:bg-[var(--bg-muted)] transition-colors">
                       <td className="p-4">
                         <div className="font-bold text-[var(--text-main)] text-sm line-clamp-1">{property.title}</div>
                         <div className="text-xs text-[var(--text-muted)] mt-1 flex items-center gap-1">
                           <MapPinIcon className="w-3 h-3" /> {property.city}
                         </div>
                       </td>
                       <td className="p-4">
                         <div className="text-xs text-[var(--text-muted)] bg-[var(--bg-muted)] px-2 py-1 rounded inline-block">
                           {property.surface} m² • {property.rooms} p.
                         </div>
                       </td>
                       <td className="p-4 text-sm font-bold text-[var(--text-main)] text-right">
                    ...
                    {properties.length === 0 && (
                     <tr>
                       <td colSpan="5" className="p-8 text-center text-[var(--text-muted)]">Aucun bien enregistré</td>
                     </tr>
                   )}
                 </tbody>
               </table>
        ...
        {activeTab === 'contracts' && (
          <section className="bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-sm overflow-hidden">
             <div className="p-6 border-b border-[var(--border-color)] flex justify-between items-center bg-[var(--bg-muted)]">
                <h2 className="text-lg font-bold text-[var(--text-main)]">Mes Contrats</h2>
                <span className="text-sm text-[var(--text-muted)]">{stats.activeContracts} actif(s)</span>
             </div>
             <div className="overflow-x-auto">
               <table className="w-full text-left border-collapse">
                 <thead>
                 ...
                 <tbody className="divide-y divide-[var(--border-color)]">
                   {contracts.map(contract => (
                     <tr key={contract.id} className="hover:bg-[var(--bg-muted)] transition-colors">
                       <td className="p-4">
                         <div className="font-bold text-[var(--text-main)] text-sm">{contract.property?.title}</div>
                         <div className="text-xs text-[var(--text-muted)] mt-1">{contract.property?.city}</div>
                       </td>
                       <td className="p-4">
                         <div className="text-sm text-[var(--text-main)]">{contract.tenant?.name}</div>
                       </td>
                       <td className="p-4 text-sm font-bold text-[var(--text-main)] text-right">
                   ...
                   {contracts.length === 0 && (
                     <tr>
                       <td colSpan="5" className="p-8 text-center text-[var(--text-muted)]">Aucun contrat trouvé</td>
                     </tr>
                   )}
                 </tbody>
               </table>
