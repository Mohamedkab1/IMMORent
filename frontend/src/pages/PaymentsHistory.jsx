...
const PaymentsHistory = () => {
  ...
  const getStatusBadge = (status) => {
    const config = {
      paid: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-500', icon: CheckCircleIcon, label: 'Payé' },
      pending: { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-500', icon: ClockIcon, label: 'En attente' },
      failed: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-500', icon: XCircleIcon, label: 'Échoué' },
      late: { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-700 dark:text-orange-500', icon: ClockIcon, label: 'En retard' }
    };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="text-3xl font-black text-text-main tracking-tight">Historique des <span className="text-primary">Paiements</span></h1>
            <p className="text-text-sub mt-1">Consultez et gérez tous les paiements effectués sur la plateforme.</p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-text-sub font-medium">Chargement des transactions...</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-bg-soft border-b border-border-main text-xs uppercase tracking-widest font-black text-text-muted">
                    <th className="p-6">Réf. Paiement</th>
                    ...
                    <tr key={payment.id} className="hover:bg-bg-soft/50 transition-colors group">
                      ...
                      <td className="p-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-text-main line-clamp-1">{payment.contract?.property?.title || 'Bien non spécifié'}</p>
                            <p className="text-xs text-text-muted">{payment.contract?.property?.city}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-6">
                        <span className="font-black text-text-main">{parseFloat(payment.amount).toLocaleString('fr-FR')} DH</span>
                      </td>
                      <td className="p-6 text-center">
                        <span className="text-xs font-semibold text-text-sub bg-bg-soft px-2 py-1 rounded-lg border border-border-main">
                          {payment.payment_method === 'card' ? 'Carte' : payment.payment_method === 'transfer' ? 'Virement' : 'Agence'}
                        </span>
                      </td>
                      <td className="p-6 text-center">
                      ...
                      <td className="p-6 text-right">
                        {payment.pdf_url ? (
                          ...
                          <span className="text-xs text-text-muted italic">Indisponible</span>
                        )}
                      </td>
                    </tr>
                  ))}
                    ...
                    <tr>
                      <td colSpan="7" className="p-20 text-center">
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <div>
                            <p className="text-lg font-bold text-text-main">Aucune transaction trouvée</p>
                            <p className="text-sm text-text-sub">Les paiements apparaîtront ici une fois effectués.</p>
                          </div>
                        </div>
                      </td>
                    </tr>
