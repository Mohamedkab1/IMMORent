...
const Messages = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] overflow-hidden bg-[var(--bg-main)]">
      
      {/* Sidebar - Liste des conversations */}
      <div className={`${showMobileList ? 'flex' : 'hidden'} md:flex flex-col w-full md:w-96 border-e border-[var(--border-color)] bg-[var(--card-bg)] transition-all`}>
        <div className="p-6 border-b border-[var(--border-color)]">
          <h1 className="text-2xl font-black text-[var(--text-main)] tracking-tight mb-4">Messages</h1>
          <div className="relative">
            <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-muted)]" />
            <input 
              type="text" 
              placeholder="Rechercher une discussion..." 
              className="w-full pl-10 pr-4 py-2 bg-[var(--bg-muted)] border-none rounded-2xl text-sm focus:ring-2 focus:ring-primary focus:bg-[var(--card-bg)] transition-all text-[var(--text-main)]"
            />
          </div>
        </div>
          ...
          {conversations.length > 0 ? (
            conversations.map(conv => (
              <button
                ...
                className={`flex items-center w-full p-4 gap-4 transition-all border-b border-[var(--border-color)] ${selectedConversation?.id === conv.id ? 'bg-primary/5 dark:bg-primary/10 border-s-4 border-s-primary' : 'hover:bg-[var(--bg-muted)]'}`}
              >
                <div className="relative flex-shrink-0">
                  <div className="w-14 h-14 rounded-2xl bg-[var(--bg-muted)] flex items-center justify-center overflow-hidden border border-[var(--border-color)]">
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex justify-between items-baseline mb-1">
                    <h4 className="font-bold text-[var(--text-main)] truncate">{conv.other_user.name}</h4>
                    <span className="text-[10px] font-medium text-[var(--text-muted)] whitespace-nowrap ml-2">
                       {conv.last_message_at ? formatDistanceToNow(new Date(conv.last_message_at), { addSuffix: true, locale: fr }) : ''}
                    </span>
                  </div>
                  <p className={`text-xs truncate ${conv.unread_count > 0 ? 'text-[var(--text-main)] font-bold' : 'text-[var(--text-muted)]'}`}>
      ...
      {/* Main Chat Area */}
      <div className={`${!showMobileList ? 'flex' : 'hidden'} md:flex flex-1 flex-col bg-[var(--card-bg)] shadow-2xl relative z-10`}>
        {selectedConversation ? (
          <>
            {/* Header */}
            <header className="px-6 py-4 border-b border-[var(--border-color)] flex items-center justify-between bg-[var(--bg-header)] backdrop-blur-md sticky top-0 z-20">
              <div className="flex items-center gap-4">
                <button onClick={() => setShowMobileList(true)} className="md:hidden p-2 hover:bg-[var(--bg-muted)] rounded-lg">
                  <ChevronLeftIcon className="w-5 h-5 text-[var(--text-main)]" />
                </button>
                <div className="w-10 h-10 rounded-full bg-[var(--bg-muted)] overflow-hidden flex-shrink-0">
                  {selectedConversation.other_user.profile_photo_url ? (
                    ...
                    <UserCircleIcon className="w-full h-full text-[var(--text-muted)]" />
                  )}
                </div>
                <div>
                  <h3 className="font-bold text-[var(--text-main)] leading-tight">{selectedConversation.other_user.name}</h3>
                  ...
                  <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] md:max-w-[70%] flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                      <div className={`px-4 py-3 rounded-2xl shadow-sm ${
                        ...
                        : 'bg-[var(--bg-muted)] text-[var(--text-main)] rounded-bl-none border border-[var(--border-color)]'
                      }`}>
            ...
            <div className="p-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
              <form onSubmit={handleSendMessage} className="flex gap-4">
                <input 
                  ...
                  className="flex-1 bg-[var(--bg-muted)] border-none rounded-2xl px-6 py-3 text-sm focus:ring-2 focus:ring-primary focus:bg-[var(--card-bg)] transition-all text-[var(--text-main)]"
                />
                <button 
                  type="submit"
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-[var(--text-muted)] bg-[var(--bg-main)]">
            <div className="w-24 h-24 bg-[var(--card-bg)] rounded-3xl flex items-center justify-center shadow-xl mb-6">
              <PaperAirplaneIcon className="w-12 h-12 text-[var(--text-muted)] -rotate-45" />
            </div>
            <h3 className="text-xl font-bold text-[var(--text-main)] mb-2">Sélectionnez une conversation</h3>
