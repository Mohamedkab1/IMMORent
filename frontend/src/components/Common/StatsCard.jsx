...
const StatsCard = ({ title, value, icon: Icon, trend, trendValue, color = "blue" }) => {
  ...
  return (
    <div className="bg-[var(--card-bg)] rounded-3xl p-6 shadow-sm border border-[var(--border-color)] hover:shadow-lg transition-all duration-300 group overflow-hidden relative">
      <div className="flex justify-between items-start relative z-10">
        <div>
          <p className="text-sm font-semibold text-[var(--text-muted)] mb-1 uppercase tracking-tight">{title}</p>
          <h3 className="text-3xl font-black text-[var(--text-main)] tracking-tighter leading-none mb-4">{value}</h3>
          ...
          {trend && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-[10px] font-medium text-[var(--text-muted)] opacity-60">vs mois dernier</span>
            </div>
          )}
        </div>
