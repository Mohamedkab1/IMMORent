...
class PropertyController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Filtre par type de transaction (sale/rent)
            if ($request->has('transaction_type') && !empty($request->transaction_type)) {
                ...
                // Support listing_type as well for compatibility
                $listingType = str_contains($request->listing_type, 'rent') ? 'rent' : 'sale';
                $query->where('transaction_type', $listingType);
            }
            ...
            if ($request->has('max_surface') && !empty($request->max_surface)) {
    ...
    private function formatProperty($property, $detailed = false)
    {
        ...
        $transactionType = $property->transaction_type ?? 'rent';
